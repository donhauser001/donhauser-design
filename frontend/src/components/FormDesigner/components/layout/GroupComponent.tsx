...
const GroupComponent: React.FC<GroupComponentProps> = ({ component, isDesignMode = false }) => {
                ...
                >
                    {component.titleDisplay !== 'hide' && (
                        <div style={{
                            ...
                            paddingBottom: '8px',
                            paddingLeft: component.titleAlign === 'left' ? '8px' : '0px',
                            paddingRight: component.titleAlign === 'right' ? '8px' : '0px',
                            textAlign: component.titleAlign || 'left'
                        }}>
                            {component.titleDisplay === 'custom' ? (component.customTitle || component.label) : component.label}
                        </div>
                    )}
                    ...
                    >
                        {childComponents.length > 0 ? (
                            isDesignMode ? (
                                // 设计模式：使用SortableContext包装，支持排序和拖拽
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                    {childComponents.map((child) => (
                                        <FormComponentRenderer key={child.id} component={child} isDesignMode={false} />
                                    ))}
                                </div>
                            )
                        ) : (

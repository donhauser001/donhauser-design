...
const testGetQuotationById = async () => {
    try {
        const quotations = await QuotationService_1.default.getAllQuotations();
        const firstQuotation = quotations[0];
        if (firstQuotation) {
            console.log(`\n🔄 测试获取报价单详情: "${firstQuotation.name}"`);
            const quotation = await QuotationService_1.default.getQuotationById(firstQuotation._id?.toString() || '');
            if (quotation) {
                console.log(`✅ 获取成功: ${quotation.name} - 默认: ${quotation.isDefault ? '是' : '否'}`);
            }
            else {
                console.log('❌ 获取失败');
            }
...
const main = async () => {
    console.log('🚀 开始测试编辑默认报价单问题...');
    await connectDB();
    await showDefaultStatus();
    await testGetQuotationById();
...
//# sourceMappingURL=testEditDefaultQuotationIssue.js.map
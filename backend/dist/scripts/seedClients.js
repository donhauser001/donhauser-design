...
const seedData = async () => {
    try {
        ...
        await ClientCategory_1.default.deleteMany({});
        console.log('📝 插入客户分类数据...');
        const insertedCategories = await ClientCategory_1.default.insertMany(clientCategories);
        console.log(`✅ 成功插入 ${insertedCategories.length} 个客户分类`);
        console.log('👥 插入客户数据...');
        const insertedClients = await Client_1.default.insertMany(clients);
        console.log(`✅ 成功插入 ${insertedClients.length} 个客户`);
        for (const category of insertedCategories) {
            const count = await Client_1.default.countDocuments({ category: category.name });
            await ClientCategory_1.default.findByIdAndUpdate(category._id, { clientCount: count });
        }
...
//# sourceMappingURL=seedClients.js.map
...
const testEditQuotationRemoveDefault = async () => {
    try {
        const quotations = await QuotationService_1.default.getAllQuotations();
        const defaultQuotation = quotations.find(q => q.isDefault);
        if (!defaultQuotation) {
            console.log('⚠️ 没有找到默认的报价单');
            return;
        }
        console.log(`\n🔄 测试编辑 "${defaultQuotation.name}" 取消默认状态...`);
        const updatedQuotation = await QuotationService_1.default.updateQuotation(defaultQuotation._id?.toString() || '', {
            name: defaultQuotation.name,
            description: defaultQuotation.description,
            isDefault: false,
            selectedServices: defaultQuotation.selectedServices,
            validUntil: defaultQuotation.validUntil
        });
        if (updatedQuotation) {
            console.log(`✅ 成功编辑报价单 "${updatedQuotation.name}" 取消默认`);
            console.log(`   默认状态: ${updatedQuotation.isDefault ? '是' : '否'}`);
        }
        else {
            console.log('❌ 编辑报价单失败');
...
const main = async () => {
    ...
    await testEditQuotationRemoveDefault();
    console.log('\n✅ 测试完成');
    process.exit(0);
};
main().catch((error) => {
...
//# sourceMappingURL=testEditDefaultQuotation.js.map